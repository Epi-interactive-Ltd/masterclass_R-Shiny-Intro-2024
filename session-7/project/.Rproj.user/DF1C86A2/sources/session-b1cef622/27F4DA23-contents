...
shinyServer(function(input, output) {
  ...
  output$alcohol_graph <- renderPlotly({
    # Get the names of the countries in the filtered data
    countries <- filtered_data()$name_long
    
    # filter 'alcohol_consumption' to include the target countries
    
    
    
    fig <- plot_ly(
      # make a line graph that uses Year (x) and Alcohol_Consumption (y)
    )
  ...
  #   data <- filtered_data()
  #   # Filter the data to be presented on the table
  #   # This last step is required
  #   st_drop_geometry()
  #   
  #   
  #   datatable(
